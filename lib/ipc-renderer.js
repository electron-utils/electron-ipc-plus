...
ipcPlus.sendToAll = function (message, ...args) {
  ...
  ipcRenderer.send.apply( ipcRenderer, ['ipc-plus:renderer2all', message, ...args] );
};
...
ipcPlus.sendToWins = function (message, ...args) {
  ...
  ipcRenderer.send.apply( ipcRenderer, ['ipc-plus:renderer2wins', message, ...args] );
};
...
ipcPlus.sendToMain = function (message, ...args) {
  ...
  if ( opts ) {
    ...
    args = ['ipc-plus:renderer2main', message, ...args, _ipcOption({
      sessionId: sessionId,
      waitForReply: true,
  ...
  if ( opts && opts.waitForReply ) {
    ...
    event.reply = function (...replyArgs) {
      if ( _debug && !_checkReplyArgs(replyArgs) ) {
        ...
        // TODO: reply an error ??
        // DISABLE: do not call return here, otherwise the reply session will never be invoked
        // return;
      }
...
ipcRenderer.on('ipc-plus:main2renderer', (event, message, ...args) => {
...
ipcRenderer.on('ipc-plus:reply', (event, ...args) => {
  let opts = _popOptions(args);
