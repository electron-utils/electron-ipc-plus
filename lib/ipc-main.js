...
if ( global.__electron_ipc_plus__ ) {
  console.warn(`A different version of electron-ipc-plus already running in the process: ${global.__electron_ipc_plus__.id}, make sure your dependencies use the same version of electron-ipc-plus.`);
}
...
global.__electron_ipc_plus__ = ipcPlus;
...
let _ErrorWinClosed = common.ErrorWinClosed;
 ...
 */
ipcPlus.removeListener = ipcMain.removeListener.bind(ipcMain);
...
ipcPlus.sendToWin = function (win, message, ...args) {
  ...
  //
  args = [win, `${moduleID}:main2renderer`, message, ...args, _ipcOption({
    sessionId: sessionId,
    waitForReply: true,
    timeout: opts.timeout, // this is only used as debug info
  })];
...
function _closeAllSessionsInWin (win) {
  let ids = Object.keys(_id2sessionInfo);
  ...
  for ( let i = 0; i < ids.length; ++i ) {
    ...
    if ( info && info.win && info.win === win ) {
      ...
      info.callback(new _ErrorWinClosed(info.message, win));
    }
  }
}
...
function _main2mainOpts (message, ...args) {
  ...
  if ( opts && opts.waitForReply ) {
    ...
    event.reply = function (...replyArgs) {
      ...
      replyArgs = [`${moduleID}:reply`, event, ...replyArgs, replyOpts];
...
ipcMain.on(`${moduleID}:renderer2all`, (event, message, ...args) => {
  let opts = _popOptions(args);
...
ipcMain.on(`${moduleID}:renderer2wins`, _renderer2renderersOpts );

ipcMain.on(`${moduleID}:renderer2main`, (event, message, ...args) => {
...
ipcMain.on(`${moduleID}:reply`, (event, ...args) => {
  let opts = _popOptions(args);
