...
let _ErrorWinClosed = ipcBase.ErrorWinClosed;
...
function _newSession ( message, prefix, fn, timeout, win ) {
  // store session window to cachedWins. Once the window closed, we will force close all sessions relate with it.
  if ( win && _cachedWins.indexOf(win) === -1 ) {
    _cachedWins.push(win);

    // close all session once the window closed
    win.once('closed', () => {
      _closeAllSessionsInWin(win, message);
      let idx = _cachedWins.indexOf(win);
      if ( idx !== -1 ) {
        _cachedWins.splice(idx,1);
      }
    });

    // TODO: what about webContents navigated ??
  }
...
function _closeAllSessionsInWin (win, message) {
  let ids = Object.keys(_id2sessionInfo);
  ...
  for ( let i = 0; i < ids.length; ++i ) {
    ...
    if ( info && info.win && info.win === win ) {
      ...
      info.callback(new _ErrorWinClosed( message, win ));
    }
  }
}
